...
Phase 22.3 adds no schema migration, Merchant/alias table, automatic mapping creation, write during resolution, transaction-history scan, fuzzy/substring/semantic match, provider confidence, Category Resolver, persistent clarification, or conversation-aware behavior. Smart Categorization's merchant-specific normalization remains responsible for mapping creation and advisory lookup; Phase 22.1 NFKC normalization remains responsible for resolution matching. Manual category input and the implemented Merchant Mapping-before-keyword precedence are unchanged. The documented Rule Engine insertion point remains reserved and unimplemented in the current backend, and Phase 22.4 Category Resolution remains pending.
