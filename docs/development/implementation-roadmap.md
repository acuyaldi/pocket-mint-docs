...
- **22.3 — Merchant Resolution:** ✅ **Implemented.** Production Merchant Resolver over owner-scoped Merchant Mapping records; provider-only `merchantReference`; trusted canonical/persisted-normalized exact matching; safe ambiguity; free-form `not_found` continuation; unchanged categorization responsibility; and unchanged explicit confirmation.
- **22.4 — Category Resolution:** ✅ **Implemented.** Production Category Resolver over authenticated owner/type-scoped private Category rows; provider-only `categoryReference`; deterministic `categoryId` compatibility; exact NFKC matching without aliases; safe ambiguity/not-found; Category-name-only previews; no seeding, fallback, schema change, or financial mutation during resolution.
- **22.5 — Clarification Engine:** 🔄 **In progress.** Persisted `ClarificationRequest`/`ClarificationOption` aggregate, digest-only single-use tokens, `PENDING/CONSUMED/CANCELLED/STALE` lifecycle, database-time 15-minute expiry (`STALE` + `terminalCode=expired`), wallet → merchant → category → draft sequential continuation inside one shared transaction, and authenticated deterministic HTTP `select`/`cancel` endpoints are implemented and PostgreSQL-verified (Tasks 1–6). Documentation alignment (Task 7) is underway. Final full-suite verification and tracked-artifact regeneration (Task 8) has not started, so the phase is not yet marked implemented.
...
### Phase 22.3 Implementation Note (2026-07-23)

The production registry now also registers `createMerchantResolver(prisma)`, exactly once. There is no standalone Merchant model, global merchant catalog, merchant lifecycle state, or merchant column on Transaction. Candidate loading therefore uses only authenticated owner-scoped `MerchantMapping` rows, selecting mapping ID, `merchantName`, and `normalizedMerchant` with the generic candidate overflow guard. Mapping IDs remain internal and cross-user-only matches are indistinguishable from `not_found`.

The provider-visible `transaction.create` contract requires textual `merchantReference` and exposes no merchant or mapping ID. The shared canonical contract keeps the reference optional for existing deterministic callers. Resolved mappings contribute only a safe merchant label; their category is never read by resolution. Ambiguity creates no draft. Because the transaction domain does not require authoritative merchant identity, `not_found` continues with validated normalized free-form text; this becomes the transaction description only when no explicit description was supplied.

Phase 22.3 adds no schema migration, Merchant/alias table, automatic mapping creation, write during resolution, transaction-history scan, fuzzy/substring/semantic match, provider confidence, Category Resolver, persistent clarification, or conversation-aware behavior. Smart Categorization's merchant-specific normalization remains responsible for mapping creation and advisory lookup; Phase 22.1 NFKC normalization remains responsible for resolution matching. Manual category input and the implemented Merchant Mapping-before-keyword precedence are unchanged. The documented Rule Engine insertion point remains reserved and unimplemented in the current backend.

### Phase 22.4 Implementation Note (2026-07-23)

The production registry now registers exactly one Category Resolver alongside Wallet and Merchant. Candidate retrieval filters `Category.userId` and trusted `Category.type` in SQL, selects only ID/name/type, and takes 101 rows for the generic fail-closed 100-candidate guard. Category name is canonical/display text and aliases are empty. Matching reuses the unchanged Phase 22.1 NFKC canonical/normalized exact evidence, integer confidence, inclusive ambiguity margin, candidate validation, stable ID tie-break, and safe public projection.

Provider `transaction.create` requires `categoryReference` and rejects Category identifiers and variants, ownership/type/confidence/evidence/trusted-constraint/lifecycle/authorization/confirmation claims, prototype and Unicode-confusable prohibited keys, and unknown fields. Canonical deterministic callers retain exactly-one-of `categoryReference | categoryId` compatibility; compatibility IDs still undergo owner/type validation. The Assistant contract remains closed to `TRANSFER`.

Wallet, optional Merchant, and Category references resolve in that order. Category ambiguity or `not_found` persists one safe clarification terminal result and creates no draft, Transaction, or balance change. Explicit Category text never falls back to Smart Categorization or Merchant Mapping category data. Draft preparation selects authoritative Category name and removes Category ID from structured/rendered previews. Confirmation remains separate, authenticated, idempotency-protected, and routed through the existing Transaction Service.

Phase 22.4 adds no schema migration, default seeding call, shared/default system Category, alias source, Rule Engine, Category management API, frontend behavior, persistent option token, Clarification Engine, conversation-aware resolution, or Phase 22.5 work. A User whose private lazy defaults have never been seeded can receive `not_found`; this remains the documented safe limitation.

### Phase 22.5 Implementation Note (2026-07-25, in progress)

Two additive PostgreSQL migrations (`20260724000000_add_clarification_engine`, `20260725000000_add_clarification_expiry`) introduce `ClarificationRequest`/`ClarificationOption` and `AssistantClarificationStatus = PENDING | CONSUMED | CANCELLED | STALE`. Ambiguous or missing wallet/category and ambiguous merchant now persist a clarification instead of terminating execution; merchant `not_found` is unchanged and still continues as free-form text. Selection is single-use and digest-only: a raw token is issued once at creation, only its SHA-256 digest is stored, and there is no reissue or recovery path. A valid selection atomically claims the request, revalidates the selected candidate and any already-resolved prerequisite, and — inside the same transaction — creates exactly one child clarification (wallet → merchant → category order, fresh 15-minute TTL, immutable context carried forward) or exactly one `PENDING_CONFIRMATION` draft. The provider is never re-invoked during continuation.

Expiry uses database time, not the application clock, and is represented as `STALE` with `terminalCode = 'expired'` rather than a separate `EXPIRED` enum value. Authenticated deterministic HTTP endpoints (`POST .../clarifications/:id/select`, `POST .../clarifications/:id/cancel`) enforce strict body validation, owner/conversation scoping indistinguishable from not-found, and the same terminal-status error table for both routes. PostgreSQL integration tests exercise same-token races, select/cancel races, and expiry races, each resolving to exactly one winner, one child-or-draft, and no financial `Transaction` or balance change.
