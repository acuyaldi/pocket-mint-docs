...
The Assistant-to-transaction relation uses `ON DELETE SET NULL`, so deleting either historical Assistant linkage or a financial transaction does not cascade-delete the other authority. There is no external call in the commit boundary and therefore no external crash window. An unexpected database failure rolls back the entire confirmation; known transaction-domain rejection marks the still-pending draft `FAILED` in a separate safe update and is never automatically retried.
