...
## 15.7. Assistant Request-Level Idempotency & Turn Recovery (Phase 27)

Phase 27 ([PD-017](../product/decisions/017-assistant-request-level-idempotency.md)) closes the one request-level idempotency gap the Assistant module itself never had: `POST /assistant/messages` and `POST /assistant/execute` accept an optional `Idempotency-Key` header, so a Web client retry, double submit, or dropped-connection resubmit can no longer create a second turn (or, for `transaction.create`, a second pending draft) from one user action. Telegram already had this guarantee through `assistantOperationGuard` (§15.5) at the channel layer and is unaffected — it never supplies this parameter.

**Claim, don't lock.** `AssistantIdempotencyRecord` (Phase 21.4, § 27) gains a nullable `turnId`, a `RUNNING | COMPLETED` status, and a stored response snapshot, and now serves both draft-confirm (`draftId` populated) and turn-level idempotency (`turnId` populated) in one table. A key is claimed with an insert-first-wins pattern — the same shape `assistantOperationGuard.beginAssistantOperation` already uses — not draft-confirm's whole-request advisory lock: `/messages` can hold an outbound provider call open for seconds (§15 Provider Adapter Boundary), and a lock held across that call was rejected as its own reliability risk. A concurrent or retried request while the first is still in flight returns `409 ASSISTANT_REQUEST_IN_PROGRESS`; a request received after the first completed replays its exact `{ httpStatus, response }` verbatim; reusing a key across operations (or against a draft-confirm key) returns `409 ASSISTANT_IDEMPOTENCY_CONFLICT`.

**Recovery-state gains `activeTurn`.** The Phase 23.5 projection (§15.3) gains a fourth field, `activeTurn` (`turnId`, `intent`, `startedAt`), sourced from the conversation's most recent `RUNNING` `AssistantTurn`. This is independent of whether the original request used an `Idempotency-Key` — a client that lost its key (or never sent one) can still discover that something is still running before deciding whether to resubmit.

**Fail-closed, not self-healing.** A process crash between claiming a key and resolving it leaves that key `RUNNING` indefinitely — the same accepted limitation `assistantOperationGuard`'s `ambiguous` outcome already carries for Telegram. This phase adds no recovery worker, dead-letter tooling, or automatic expiry. See PD-017 for the full decision record, including why draft-confirm's advisory-lock pattern was rejected for this case specifically.

## 15.8. Channel Source Attribution (Phase 30)

Phase 30 ([PD-020](../product/decisions/020-channel-source-attribution.md)) closes the one transparency gap left after Phases 25–29 made cross-channel conversations reliable and operable: nothing in the conversation/turn DTOs told a Web client that a given turn actually originated from a linked Telegram chat. `AssistantTurn` gains an `AssistantChannel` (`WEB | TELEGRAM`) column, defaulting to `WEB`; `getOwnedConversation` exposes it per turn plus a derived `conversation.sourceChannels`, and `listOwnedConversations` exposes the same `sourceChannels` per summary row via one additional `groupBy` query. Only these two safe enum values ever cross the API boundary — never `externalChatId`, `externalUserId`, `externalSenderId`, a callback token, or any other provider identifier.

**Stamped after the fact, never threaded through shared application code.** `beginTurn` and `BeginTurnInput` are unchanged — every turn is created `WEB` by default, exactly as before. `src/channels/workers/inbound.worker.ts` already learns the id of the turn it just produced, for both the MESSAGE path (`processJob`) and the CALLBACK path (`processCallbackJob`); it now updates that one turn's `channel` to `TELEGRAM` immediately afterward. `application.service.ts`, `provider-runtime.ts`, and `financial-draft.service.ts` — the exact surface Phase 26A (§15.6) built so a Telegram callback and the Web client call the same clarification/draft methods with "no parallel logic" — are not modified by this phase. `renderApplicationResult` (§15.6) gains one optional `turnId` field on its `CallbackActionResult` return shape so the CALLBACK path can find the turn id it needs to stamp; this is the only change to that shared code, and it is additive. The stamp is best-effort — a failed update is caught, logged, and never blocks reply delivery or job success, since channel attribution is presentation metadata, not a delivery or financial guarantee.

**Durable going forward, bounded gap in old data.** A pure read-time derivation (joining a turn to `ChannelInboundJob.assistantTurnId`) was rejected: `ChannelInboundJob` rows are retention-purged (§15.5, `src/channels/retention.ts`, 7/28 days), so attribution would silently decay back to `WEB` as jobs aged out, and a CALLBACK-originated turn was never linked via `assistantTurnId` at all before this phase — that approach would have permanently misattributed every Telegram callback turn, not just old ones. The persisted-column design has neither problem for any turn created after this phase ships. A one-time migration backfill retags turns whose `ChannelInboundJob` has not yet been purged; anything older, and every pre-Phase-30 callback-originated turn, keeps the `WEB` default — a documented, bounded, one-time gap in historical data, not an ongoing one. See PD-020 for the full decision record, including why threading a `channel` parameter through the shared application/provider-runtime surface and an `AsyncLocalStorage`-based ambient-context alternative were both rejected in favor of this narrower stamp.

## 15.9. Channel Delivery Receipts (Phase 31)

Phase 31 ([PD-021](../product/decisions/021-channel-delivery-receipts.md)) surfaces the delivery lifecycle the channel worker pipeline (§15.5) already tracks in `ChannelOutboundDelivery.status`, so a Telegram-originated turn's Assistant reply shows the user whether it is still delivering, has been sent, or terminally failed — closing the gap Phase 30 (§15.8) left after making *where a turn came from* visible without also making *whether its reply arrived* visible. `getOwnedConversation` gains a computed `turn.deliveryStatus`, mapped from the closed internal `ChannelDeliveryStatus` enum to a safe, closed `AssistantDeliveryStatus`: `PENDING → PENDING`, `SENDING → PROCESSING`, `SENT → DELIVERED`, `FAILED_RETRYABLE → PROCESSING` (still within its bounded retry/backoff schedule, not yet a real failure), `FAILED_TERMINAL → FAILED` (the state Phase 29's operator remediation, §15.7, already exists to act on). A WEB turn reads `NOT_APPLICABLE`; a TELEGRAM turn whose delivery row has aged past retention (§15.5) reads `UNKNOWN` (Phase 32, §15.10) — never guessed, and never omitted on a current backend.

**Read-time, not stamped.** Unlike `channel` — a historical fact that needed a durable column specifically because `ChannelInboundJob` retention purge would otherwise let attribution decay — delivery status is live state that already has one authoritative source. `getOwnedConversation` adds one bounded query scoped to the page's TELEGRAM-channel turn ids (`channelInboundJob.findMany` joined to its `SEND_MESSAGE`-kind delivery), the same cost profile as Phase 30's `sourceChannels` groupBy. No migration, no worker change, no write path touched. `listOwnedConversations` is unchanged — delivery status is a per-turn concept and the summary list does not return turns.

Only the closed enum crosses the API boundary — never `providerMessageId`, `destinationChatId`, `attempt`, reply markup, or any text beyond the Assistant message's own already-visible `content`. See PD-021 for the full decision record, including why the field lives on the turn (mirroring `channel`) rather than the message, and why a persisted column and a user-facing retry action were both rejected.

## 15.10. Channel Delivery Retention & Historical UX Semantics (Phase 32)

Phase 31 (§15.9) omitted `deliveryStatus` for a TELEGRAM turn whose `ChannelOutboundDelivery` row had already been retention-purged (§15.5) — safe, but indistinguishable on the wire from a pre-Phase-31 backend that has no concept of delivery status at all; both produce a response with the key simply absent. Phase 32 ([PD-022](../product/decisions/022-channel-delivery-retention.md)) closes that ambiguity: `AssistantDeliveryStatus` gains a sixth value, `UNKNOWN`, and `deliveryStatus` is now always present on a current backend response — never omitted. `resolveDeliveryStatus(channel, mappedStatus)` (`conversation.service.ts`) makes the three-way split explicit: `NOT_APPLICABLE` for `WEB`, the Phase 31 mapped status for a `TELEGRAM` turn with a retained delivery row, `UNKNOWN` for a `TELEGRAM` turn with none.

No retention policy change — `src/channels/retention.ts` and `CHANNEL_RETENTION_DAYS` are untouched; this phase only makes the read path's existing behavior once a row ages out explicit and typed, instead of an omitted key. The frontend renders nothing for `UNKNOWN`, identically to `NOT_APPLICABLE` and the old absent-field case: a historical-unknown status carries no actionable information, and rendering nothing is what guarantees it can never be mistaken for a failure, per Phase 32's own hard requirement. The existing `AssistantMessage` rendering switch's `default` arm already produced exactly this behavior for an unrecognized value, so no new frontend render branch was needed — only its `AssistantDeliveryStatus` type was widened to name the value. See PD-022 for the full decision record, including why a second `deliveryStatusRetained` boolean field was rejected in favor of one additional enum value.
